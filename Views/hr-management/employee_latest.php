...
$('#employee_data_table_add_new').on('click', function() {
    alert("inside employee_data_table_add_new ");
    // clearInputFieldsAndTextareas();
    ...
    $.ajax({
        ...
        success: function(data) {
           console.log("fetchEmployeeNames",data)
            ...
            dropdown9.empty();
            dropdown9.append($('<option></option>').attr('value', '0').text('Not Applicable')); 
            dropdown10.empty();
            dropdown11.empty();
...
alert(selectedBranchId);
    ...
    $('#employee_department_id').change(function() {
        ...
        url=BASE_URL + "index.php/" + hrController + "/get_employee_heads_by_department";
        alert(url);
            $.ajax({
                url: BASE_URL + "index.php/" + hrController + "/get_employee_heads_by_department",
                type: 'POST',
                ...
                success: function(response) {
                    console.log(response);
                    $('#employee_head_id_in_employee').empty().append($('<option></option>').val('').text('Select Employee Head'));

                    // Append employee heads obtained from the AJAX call
                    $.each(response, function(index, employee) {
                        $('#employee_head_id_in_employee').append(
                            $('<option></option>').val(employee.id).text(employee.employee_name)
                        );
                    });
                },
                error: function(xhr, status, error) {
